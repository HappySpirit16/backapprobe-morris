	...
	public String handleRequest(Request input, Context context) {
		...
		case "dispersarPagar":
			...
			try {
				...
				System.out.println("clientReceive" + clientReceive);
				System.out.println("approbeDispersar" + approbeDispersar);
				System.out.println("clientReceive" + transferenciaPowwi[0].getMonto());
				System.out.println("approbeDispersar" + transferenciaPowwi[1].getMonto());

				System.out.println("vector" + dispersar.getTransferenciaPowwi());
				mapper.save(basicInformation);
				...
				if (Arrays.asList(responseTransaction).contains("{\"codigoResultado\":\"TBA_567\"")) {
					...
					for (int i = 0; i < responseTx.getTransacciones().length; i++) {
						...
						convertedTx = (" + "+responseTx.getTransacciones()[i].getValorTotalTransaccion())  + convertedTx;
					}
					...
					saveRelation1.put("activityBorrower",borrowerUpdate.getCreditInformation().getProfession().getProfession());
					...
					saveRelation1.put("channel","P2P");
					saveRelation1.put("creditType",borrowerUpdate.getCreditInformation().getPurpose().getDescription());
					...
					saveRelation2.put("bussinesName",borrowerUpdate.getCreditInformation().getLaborInformation().getCompanyName());
					...
					saveRelation2.put("department",borrowerUpdate.getGeneralInformation().getDepartment().getDepartment());
					saveRelation2.put("district",borrowerUpdate.getGeneralInformation().getDepartment().getDepartment());
...
//					""+oApprobe, input.getTransferenciaPowwi()[0].getReferencia1(), input.getTransferenciaPowwi()[0].getReferencia2());
		...
		case "pagar":
			...
			QuerySpec spec1 = new QuerySpec().withProjectionExpression("numberIdBorrower, numberIdMoneylender")
